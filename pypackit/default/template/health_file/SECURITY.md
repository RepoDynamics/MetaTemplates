# {metadata[project][name]} Security and Vulnerability Disclosure Policy

{metadata[project][name]} takes the security and privacy of all its users and members very seriously,
...
{metadata[url][security_measures]}
...
or [**click here to directly open a private report form**]({metadata[url][github][security][new_advisory]}).

<blockquote>
    🥷🏾 <b>Prefer Anonymity?</b>
    <br><br>
    Our vulnerability management team also welcomes anonymous emails sent to
    <a href="mailto:{metadata[maintainer][email][security]}
?subject=
...
">{metadata[maintainer][email][security]}</a>.
</blockquote>
...
{metadata[project][name]} is a free and open-source project.
...
* Source code at {metadata[url][github][home]}
...
please contact us at {metadata[maintainer][email][security]} to discuss it first, before starting your research.
